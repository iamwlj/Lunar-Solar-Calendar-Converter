...
local function SolarFromInt(g)
    ...
    local mi = math.floor((100 * ddd + 52) / 3060)
    local mm = (mi + 2) % 12 + 1
    y = y + math.floor((mi + 2) / 12)
    local dd = ddd - math.floor((mi * 306 + 5) / 10) + 1
    local solar = {
            year = y,
            month = mm,
            day = dd,
...
function LunarToSolar(lunar)
    local days = lunar_month_days[lunar.year - lunar_month_days[1]+1]
    local leap = GetBitInt(days, 4, 13)
    local offset = 0
    local loopend = leap
    if not lunar.isleap then
        if lunar.month <= leap or leap == 0 then
    ...
    for i=0,loopend-1 do
        offset = offset + (GetBitInt(days, 1, 12 - i) == 1 and 30 or 29)
    end
    offset = lunar.day + offset

    local solar11 = solar_1_1[lunar.year - solar_1_1[1]+1]

    local y = GetBitInt(solar11, 12, 9)
    local m = GetBitInt(solar11, 4, 5)
    local d = GetBitInt(solar11, 5, 0)
...
function SolarToLunar(solar)
    local lunar = {}
    local index = solar.year - solar_1_1[1]
    local data = bit32.bor(bit32.lshift(solar.year,9), bit32.lshift(solar.month, 5),solar.day)
    local solar11 = 0
    if solar_1_1[index+1] > data then
        index = index - 1
    end
    local solar11 = solar_1_1[index+1]
    local y = GetBitInt(solar11, 12, 9)
    local m = GetBitInt(solar11, 4, 5)
    local d = GetBitInt(solar11, 5, 0)
    local offset = SolarToInt(solar.year, solar.month, solar.day) - SolarToInt(y, m, d)

    local days = lunar_month_days[index+1]
    local leap = GetBitInt(days, 4, 13)

    local lunarY = index + solar_1_1[1]
    local lunarM = 1
    local lunarD
    offset = offset + 1

    for i=0,12 do
        local dm = GetBitInt(days, 1, 12 - i) == 1 and 30 or 29
        if offset > dm then
            lunarM = lunarM + 1
            offset = offset - dm
        else
    ...
    lunar.month = lunarM
    lunar.isleap = false
    if leap ~= 0 and lunarM > leap then
        lunar.month = lunarM - 1
        if lunarM == leap + 1 then
            lunar.isleap = true
        end
    end
